...
{
    /**************************************标定参数********************************************/
    Mat cameraMatrixL = (Mat_<double>(3, 3) << 2797.8, 0, 672.2,
                         0, 2788.9, 332.6,
                         0, 0, 1);
    Mat distCoeffL = (Mat_<double>(5, 1) << -1.0649, 4.7557,0.0185 ,0.0263,0.0);

    Mat cameraMatrixR = (Mat_<double>(3, 3) << 2612, 0, 692.4,
        0, 2603, 294.1,
        0, 0, 1);
    Mat distCoeffR = (Mat_<double>(5, 1) << -0.2956, -5.0048, 0.0127, 0.011 , 0.0);

    Mat T = (Mat_<double>(3, 1) << -197.6388 ,-6.2445 ,-80.4236);//T平移向量
    Mat R = (Mat_<double>(3, 3) << 0.9972, -0.0105,  -0.0923,
             0.0169, 0.9961, 0.0861,
             0.0912, -0.0871,  0.9935);//R旋转向量
