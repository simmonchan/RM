...
#include "include/Camera/Structure_Right.h"
FILE *flog;
struct shared_package_right *shared_package_ptr = NULL;
using namespace std;
using namespace cv;
long long frame_count_last = 0;
uint8_t image_buffer[921600];
int image_buffer_len;
struct shared_package_right *get_shared_package()
{
    int shmid;
    shmid = shmget(960826, sizeof(struct shared_package_right), 0666 | IPC_CREAT);
    struct shared_package_right *s = (struct shared_package_right *) shmat(shmid, NULL, 0);
    printf("shmid:%d, p:%lx\n", shmid, s);
    return s;
}
uint8_t getImageFromMemory_right(Mat &image)
{
