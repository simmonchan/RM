...
typedef struct
{
    ...
    int flag;//设置1表示目标进入了可以开火的范围，设置0则表示目标尚未进入可开火的范围，目前暂不使用，默认置0
    int isFingTarget;//当识别的图片范围内有目标且电控发来的信号不为0x00（关闭视觉）置为1，否则置0
} VisionData;
...
public:
    ...
    void TransformData(const VisionData &data); //方案2
    void send();
    void closePort();


    void TransformData_01(int pitch, int yaw, int dis);//方案1
